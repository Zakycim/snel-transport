...
public class OrderListFacade extends AbstractFacade<OrderList> {
    ...
    public void AssignOrdersTolist() {
        ...
        OrderList temporalList1 = new OrderList();// = em.find(OrderList.class,
                                                  // 1L);
        int idCounter = 1;

        OrdersFacade of = new OrdersFacade();
        List<Orders> orders = of.getOrdersByDates();
        ...
        for (int i = 0; i < idorders.size(); i++) {
            ...
            // System.out.println("hier stop ik"+ of.find(idorders.get(i)) +
            // "size:" + idorders.size());
            ...
            // tx.begin();
            // Query query = em.createQuery("INSERT OrderList o VALUES (orderID,
            // '', 2L)" );
            // query.setParameter("orderID", orderID);
            // temporalList1.setId((long) orderListSize);
            // temporalList1 = em.find(OrderList.class, new Long
            // (orderListSize+1));
                ...
                for (idCounter = 1; idCounter <= orderListSize; idCounter++) {
                    if ((availableTime > 0) && (idCounter >= lastOrderlistId)) {
                        ...
                        try {
                            ...
                            // OrderList temporalList2 =
                            // em.find(OrderList.class, new Long(idCounter));
